---
title: Exploring the Accrual Landscape^[We appreciate feedback from Ulf Brüggemann, Maximilian Muhn, Jochen Pierk, Harm Schütt as well as from the audiences of seminars at Bristol, Exeter, the AMIS conference in Bucharest, the 2018 EAA Doctoral Colloquium and HU Berlin. The authors can be reached via gassen@wiwi.hu-berlin.de.] \vspace{2cm}
...
pkgs <- c("knitr", "kableExtra", "captioner", 
          "devtools", "Quandl", "gtools", "ggpubr", "lfe",
...
vt <- data.frame(Variable=vd$var_name, Definition=vd$var_def, stringsAsFactors = FALSE)
vt <- vt[c(7:12, 6, 5, 13, 14),]
vt$Definition[1] <- "0.5*lag(at) + 0.5*at"
vt$Definition[4] <- "(act - lag(act) - (che - lag(che)) - ((lct - lag(lct)) - (dlc - lag(dlc))) - dp)/avg_at"
...
latex_tab <- c("\\begin{table}[!htbp]",  
               ...
               "\\setlength{\\labelsep}{0pt}",
               "\\footnotetext",
               "\\item Note: This reproduced sample is based on a Compustat North America dump and the screens as reported by BLZ, p. 49f. Variables are as defined in Table \\ref{tab:vardef}.",
               "\\end{tablenotes}",
               "\\end{threeparttable}",
               "\\end{table}")
cat(paste(latex_tab, collapse = "\n"))  
...
latex_tab <- c("\\begin{table}[!htbp]",  
               ...
               "\\setlength{\\labelsep}{0pt}",
               "\\footnotetext",
               "\\item Note: The test sample is based on the reproduced sample reported in Table \\ref{tab:descriptives_rs}. In addition, observations with $avg\\_at$ below M US-\\$ 7.5 are excluded. Variables are as defined in Table \\ref{tab:vardef}.",
               "\\end{tablenotes}",
               "\\end{threeparttable}",
               "\\end{table}")
cat(paste(latex_tab, collapse = "\n"))  
...
``` {r level_by_ind, cache = TRUE, fig.cap="\\label{fig:level_by_ind}$time$ coefficient of model (\\ref{time_reg}) by Fama/French industries"}
...
``` {r scatter_sbs, cache = TRUE, dev = "png", fig.cap="\\label{fig:scatter_sbs} $CFO$ and $TACC$ scatter plots (sample of 20,000 observations for each period)"}
...
dist_test[[1]]$table[12] <- "\\\\[-1.8ex] & cfo\\_est & cfo\\_est & cfo\\_est & cfo\\_est & cfo\\_est & cfo\\_est & resid\\_cfo \\\\ "
latex_tab <- c("\\begin{table}[!htbp]",
...
dist_test[[2]]$table[12] <- "\\\\[-1.8ex] & adjr2 & adjr2 & adjr2 & adjr2 & adjr2 & adjr2 & resid\\_adjr2 \\\\ "
latex_tab <- c("\\begin{table}[!htbp]",  
...
5. You will need Java to be installed to your system if you want access WRDS remotely (https://www.java.com/).
6. If you have a Github account, you can link RStudio to it (follow the instructions under http://happygitwithr.com/) and then fork the Github repository https://github.com/joachim-gassen/ExpAcc
7. If you do not have a Github account (yet), download the ZIP-File containing the repository for this paper from https://github.com/joachim-gassen/ExpAcc and extract it on your computer (click the top right green button named "clone or download").
...
9. In the "code" sub-directory, you will find a file named "fetch_wrds_data.R". Open it. The R code will open in the upper left corner of the screen. No worries if you never worked with R before. You only need to adjust a few things to adjust the code to your WRDS subscription. 
10. In order to fetch the data from WRDS you first need to encode your WRDS password by using SAS commands. If you have not installed SAS locally, you can run the SAS commands via the WRDS cloud (see the instruction here: https://wrds-www.wharton.upenn.edu/pages/support/wrds-cloud/connecting-wrds-cloud/using-ssh-connect-wrds-cloud/). Once you have connected to the WRDS Cloud via SSH, follow the PROC PWENCODE procedure as explained here https://wrds-web.wharton.upenn.edu/wrds/support/Accessing%20and%20Manipulating%20the%20Data/_010Encoding%20your%20WRDS%20Password.cfm.
11. After you have successfully encoded your password, assign the encoded password to the variable "pass" in "fetch_wrds_data.R". Your user name is stored in the "user" variable.
12. Download and save the SAS JDBC drivers as explained under the section "Preparing your environment - R Studio" of the following link: https://wrds-web.wharton.upenn.edu/wrds/support/Accessing%20and%20Manipulating%20the%20Data/_007R%20Programming/_001Using%20R%20with%20WRDS.cfm.  Save the SAS JDBC drivers and assign the respective path to the variable "wrds_path" in "fetch_wrds_data.R" (make sure that you use normal slashes "/" instead of backslashes for assigning the path).
13. Save the changes that you have made to "fetch_wrds_data.R". You should now be able to download the necessary data from WRDS by sourcing "fetch_wrds_data.R" (Ctrl + Shift + Enter). Congratulations!
14. Open the file "produce_findings.R" in the main directory. Run all (Ctrl + A, Ctrl + Enter). This will install necessary R packages, including the ExPanDaR package, generate all samples, tables and figures that are presented in the paper. In the end, it will open a local version of the ExPanD app that is also available online (https://jgassen.shinyapps.io/ExpAcc/).
15. If you want to reproduce the whole paper, open the file ExpAcc_paper.Rmd in the paper directory and knit it (Ctrl + Shift + K, Icon in the tray above the text editor). Note: You will need to have some LaTeX distribution installed on your system for knit to produce PDF files.
16. Where do you go from here? Well that depends. A good first step would be to explore and audit the code of the repository. It is contained in the code directory. The naming of the files should be somewhat self-explanatory. As an example for what to do: Experiment with the sample screens and the outlier treatment of the respective samples.
17. If you stumble across something interesting, please get in touch. The most efficient way to do that would be to open up an issue on Github or to issue a pull request with your suggested changes. But we also cater to traditional email. Just reach out to gassen@wiwi.hu-berlin.de
