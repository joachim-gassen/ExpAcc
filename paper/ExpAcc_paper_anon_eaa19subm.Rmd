---
title: Exploring the Accrual Landscape by Open Science^[This is an anonymized version of a current working paper. While it contains no identity revealing information, following the links included in the paper might reveal the identity of the author(s). Regardless, as the online materials are a central part of the paper, the links have not been removed. Reviewer discretion is advised.] \vspace{4cm}
...
Note: This Appendix is not functional in the anonymous version.
...
6. If you have a Github account, you can link RStudio to it (follow the instructions under http://happygitwithr.com/) and then fork the Github repository https://github.com/XXXX/ExpAcc as a new project. Continue with step 14.
7. If you do not have a Github account, download the ZIP-File containing the repository for this paper from https://github.com/XXXX/ExpAcc and extract it on your computer (click the top right green button named "clone or download").
...
11. Issue the following command to pull the container from Docker Hub and run it in your environment: `docker run -d -p 8787:8787 -e PASSWORD=yourpass --name expacc XXXX/expacc`
...
18. If you stumble across something interesting, please get in touch. The most efficient way to do that would be to open an issue on Github or to issue a pull request with your suggested changes. But we also cater to traditional email. Just reach out to XXXX
